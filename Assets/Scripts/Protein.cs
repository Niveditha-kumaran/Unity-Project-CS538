...
public class Protein : MonoBehaviour
{
    ...
    public int owner; // player that owns this protein

    // Dissolve
    public Dissolve Dissolve;
    private bool destroyed = false;
    private bool isBeingThrown = false;
    private float v_init = 0f;
        ...
        else if (proteinManager.PlayerIsHost())
        {
            ...
            if (collision.gameObject != wall) //&& collision.gameObject != floor)
            {
                ...
                if (GetProteinGroup(this.name) == GetProteinGroup(collidedProtein.name)){
                    ...
                    effectPositive.transform.parent = this.transform;
    ...
    void Bounce()
    {
        ...
        if (this.transform.position.y <= minBounce && bounceDir == -1)
        {
            bounceTime = 0;
            bounceDir = 1;
            v_init = 3f;
            x_init = this.transform.position.y;
        }

        if (0.5 * grav * bounceTime * bounceTime + this.v_init * bounceTime < 0)
        {
            bounceDir = -1;
        }
        ...
        float y = (float)(x_init + (0.5 * grav * bounceTime * bounceTime) + this.v_init * bounceTime);
